...
function filterBadges(list) {
  ...
  badges.forEach((badge) => {
    ...
    const safeName = canonical || normalizeBadge(badge);
...
function fetchJson(url) {
  return new Promise((resolve, reject) => {
    ...
    const req = client.get(url, {
      headers: { 'User-Agent': 'fakewanted-bio/1.0', 'Accept': 'application/json' },
      timeout: 10000,
      // Force IPv4 to avoid hosts where IPv6 egress is blocked
...
function defaultProfile() {
  return {
    ...
    badgeGlow: 12,
    badgeHideBackground: false,
    badgeDisableAnimation: false,
    badgeShape: 'rounded',
    badgeSize: 38,
    badgeNoOutline: false,
    badgeVisibility: [],
    tags: [],
    hideTags: false,
    showWatermark: true,
    ...
    seoDescription: '',
    discordInvite: '',
    discordWidgetPosition: 'belowBadges',
    discordWidgetGlow: false,
    discordBgOpacity: 100,
    discordGlowOpacity: 40,
    audioReactive: false
  };
}
...
// AI-like heuristic shield for suspicious traffic
const aiShieldBuckets = new Map(); // ip -> { count, reset }
function aiShield(req, res, next) {
  const ip = (req.headers['x-forwarded-for'] || req.socket.remoteAddress || '').split(',')[0].trim();
  const ua = (req.headers['user-agent'] || '').toLowerCase();
  const now = Date.now();
  const bucket = aiShieldBuckets.get(ip) || { count: 0, reset: now + 60 * 60 * 1000 };
  if (now > bucket.reset) {
    bucket.count = 0;
    bucket.reset = now + 60 * 60 * 1000;
  }
  if (/curl|python|scrapy|bot|crawler|spider|wget|go-http-client/.test(ua)) {
    bucket.count += 2;
  }
  if (req.method === 'POST' && (req.path.includes('login') || req.path.includes('register'))) {
    bucket.count += 1;
  }
  aiShieldBuckets.set(ip, bucket);
  if (bucket.count > 20) {
    return res.status(403).json({ error: 'AI shield: suspicious activity detected' });
  }
  next();
}
app.use(aiShield);
...
app.post('/api/register', async (req, res) => {
  ...
  const ref = (req.query.ref || req.body.ref || '').toString().trim().toLowerCase();
  ...
  try {
    ...
    users.push(user);
    // referral tracking and Inviter badge
    if (ref) {
      const refUser = users.find((u) => u.username === ref);
      if (refUser && refUser.id !== user.id) {
        refUser.referrals = (refUser.referrals || 0) + 1;
        if (refUser.referrals >= 10) {
          if (!refUser.badges.includes('Inviter')) {
            refUser.badges.push('Inviter');
          }
        }
      }
    }
...
app.post('/api/rewards/redeem', requireAuth, (req, res) => {
  ...
  if (Array.isArray(reward.badges)) {
    ...
    if (reward.badges.length) bonus.badge = reward.badges.join(', ');
  }
  ...
  if (reward.customBadgeSlots && Number(reward.customBadgeSlots) > 0) {
    ...
    bonus.customBadgeSlots = Number(reward.customBadgeSlots);
  }

  if (reward.allowOneLetter) {
    user.allowOneLetter = true;
    bonus.title = 'One-letter username access';
  }
  ...
  req.session.user = { id: user.id, username: user.username, role: user.role, premium: isPremiumActive(user) };
  res.json({ success: true, user: sanitizeUser(users[userIdx]), reward: rewards[idx], bonus });
});
...
app.post('/api/profile/save', requireAuth, (req, res) => {
  ...
  const layoutTemplates = ['default', 'modern', 'simplistic', 'portfolio', 'poster', 'compact', 'fast', 'glow', 'emerald'];
  ...
  allowedProfile.audioReactive = false;
  ...
  allowedProfile.discordInvite = '';
  allowedProfile.discordWidgetPosition = 'belowBadges';
  allowedProfile.discordWidgetGlow = false;
  allowedProfile.discordGlowOpacity = 0;
  allowedProfile.discordBgOpacity = 0;
  allowedProfile.showUidTooltip = incomingProfile.showUidTooltip === undefined
    ? (user.profile.showUidTooltip ?? true)
    : Boolean(incomingProfile.showUidTooltip);
  allowedProfile.showUidInline = incomingProfile.showUidInline === undefined
    ? (user.profile.showUidInline ?? false)
    : Boolean(incomingProfile.showUidInline);
  ...
  const rawBoxBlur = Number(incomingProfile.boxBlur);
  allowedProfile.boxBlur = Number.isFinite(rawBoxBlur)
    ? Math.max(0, Math.min(50, rawBoxBlur))
    : (Number.isFinite(user.profile.boxBlur) ? user.profile.boxBlur : 0);
  ...
  allowedProfile.badgeHideBackground = Boolean(incomingProfile.badgeHideBackground);
  allowedProfile.badgeDisableAnimation = Boolean(incomingProfile.badgeDisableAnimation);
  ...
  const rawBadgeSize = Number(incomingProfile.badgeSize);
  allowedProfile.badgeSize = Number.isFinite(rawBadgeSize) ? Math.min(80, Math.max(20, rawBadgeSize)) : user.profile.badgeSize || 38;
  allowedProfile.badgeNoOutline = Boolean(incomingProfile.badgeNoOutline);
  allowedProfile.showWatermark = isPremiumActive(user) ? incomingProfile.showWatermark !== false : true;
  allowedProfile.seoAnimation = typeof incomingProfile.seoAnimation === 'string' ? incomingProfile.seoAnimation : user.profile.seoAnimation || 'none';
  allowedProfile.seoCardTitle = typeof incomingProfile.seoCardTitle === 'string' ? incomingProfile.seoCardTitle : user.profile.seoCardTitle || '';
  const rawCardScale = Number(incomingProfile.cardScale);
  allowedProfile.cardScale = Number.isFinite(rawCardScale) ? Math.min(1.3, Math.max(0.8, rawCardScale)) : user.profile.cardScale || 1;
  allowedProfile.badgeCardEnabled = Boolean(incomingProfile.badgeCardEnabled);
  allowedProfile.badgeCardLabel = typeof incomingProfile.badgeCardLabel === 'string' ? incomingProfile.badgeCardLabel : user.profile.badgeCardLabel || '';
  const maxTags = isPremiumActive(user) ? 10 : 5;
  allowedProfile.tags = Array.isArray(incomingProfile.tags)
    ? incomingProfile.tags.filter(Boolean).map(t => String(t).trim()).slice(0, maxTags)
    : (user.profile.tags || []).slice(0, maxTags);
  const allowedTagPositions = ['below-name', 'above-name', 'top'];
  allowedProfile.tagPosition = allowedTagPositions.includes(incomingProfile.tagPosition)
    ? incomingProfile.tagPosition
    : user.profile.tagPosition || 'below-name';
  allowedProfile.hideTags = Boolean(incomingProfile.hideTags);
...
app.get('/api/discord/:invite', async (req, res) => {
  const raw = String(req.params.invite || '').trim();
  const code = raw
  ...
  const proxy = (process.env.DISCORD_API_PROXY || '').trim().replace(/\/$/, '');
  const directUrl = `https://discord.com/api/v10/invites/${encodeURIComponent(code)}?with_counts=true&with_expiration=false`;
  const url = proxy ? `${proxy}/${encodeURIComponent(code)}?with_counts=true&with_expiration=false` : directUrl;
  try {
    let data;
    try {
      data = await fetchJson(url);
    } catch (err) {
      // If proxy failed, try direct as a fallback; if direct failed, bubble error
      if (proxy) {
    ...
    return res.json({
      name: data?.guild?.name || 'Discord server',
      id: data?.guild?.id || '',
    ...
    // Fallback when Discord is blocked: still return a minimal payload so the widget can render a join button
    return res.json({
      name: 'Discord server',
      id: '',
      icon: '',
      ...
      invite: code,
      fallback: true,
      error: 'Failed to fetch Discord invite (network blocked?)'
    });
  }
});
...
app.get('/api/admin/ai-status', (req, res) => {
  const summary = [];
  aiShieldBuckets.forEach((v, k) => summary.push({ ip: k, count: v.count, reset: v.reset }));
  res.json({ active: true, buckets: summary });
});
...
// Admin: adjust profile views (add/remove)
app.post('/api/admin/users/:username/views', (req, res) => {
  ...
  if (isAdminUser(target)) {
    return res.status(400).json({ error: 'Cannot modify admin views' });
  }
  const current = Number(target.views || 0);
  target.views = Math.max(0, current + delta);
  users[idx] = normalizeUser(target);
  writeUsers(users);
  res.json({ success: true, views: target.views, user: sanitizeUser(users[idx]) });
});
