...
app.post('/api/profile/save', requireAuth, (req, res) => {
  ...
  allowedProfile.audioReactive = incomingProfile.audioReactive === undefined
    ? (user.profile.audioReactive ?? false)
    : Boolean(incomingProfile.audioReactive);
  ...
  const inviteRaw = typeof incomingProfile.discordInvite === 'string' ? incomingProfile.discordInvite.trim() : '';
  const cleanedInvite = inviteRaw
    .replace(/^https?:\/\/(?:www\.)?discord(?:app)?\.com\/invite\//i, '')
    .replace(/^https?:\/\/(?:www\.)?discord\.gg\//i, '')
    .replace(/^\//, '')
    .split('?')[0];
  allowedProfile.discordInvite = cleanedInvite ? cleanedInvite.slice(0, 64) : (user.profile.discordInvite || '');
  const widgetPositions = ['belowBadges', 'belowBio', 'belowLinks', 'belowAudio'];
  allowedProfile.discordWidgetPosition = widgetPositions.includes(incomingProfile.discordWidgetPosition)
    ? incomingProfile.discordWidgetPosition
    : (user.profile.discordWidgetPosition || 'belowBadges');
  allowedProfile.discordWidgetGlow = Boolean(incomingProfile.discordWidgetGlow);
  const rawDiscordGlow = Number(incomingProfile.discordGlowOpacity);
  allowedProfile.discordGlowOpacity = Number.isFinite(rawDiscordGlow)
    ? Math.max(0, Math.min(100, rawDiscordGlow))
    : (Number.isFinite(user.profile.discordGlowOpacity) ? user.profile.discordGlowOpacity : 40);
  const rawDiscordBg = Number(incomingProfile.discordBgOpacity);
  allowedProfile.discordBgOpacity = Number.isFinite(rawDiscordBg)
    ? Math.max(0, Math.min(100, rawDiscordBg))
    : (Number.isFinite(user.profile.discordBgOpacity) ? user.profile.discordBgOpacity : 100);
